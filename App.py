...
def display_move(result, title): flavor_texts = { "Weapon": "Forged in steel and sharpened in battle, this technique strikes with precision and mythic force.", "Grappling": "Twist, lock, and crash—this move seizes the body like a tidal surge of raw control.", "Unarmed": "A blur of fists and fury, born from breath, balance, and elemental mastery." } flavor = flavor_texts.get(result['Category'], "A mysterious force flows through this motion.")
...
martial_quotes = [
    ...
    "When the path is blocked, redirect.",
    "The body follows where the mind dares to go.",
    "A warrior trains even when the battlefield is still.",
    "Precision beats power. Timing beats speed.",
    "In stillness, there is control. In motion, there is freedom.",
    "Fall seven times, rise eight. — Japanese Proverb",
    "Strength without control is chaos.",
    "Fear is the only true enemy of the martial artist.",
    "Train not until you get it right. Train until you cannot get it wrong.",
    "Technique conquers strength.",
    "The more you sweat in training, the less you bleed in combat.",
    "Balance is not something you find. It’s something you create.",
    "Courage is the spirit that refuses to back down.",
    "Strike with purpose, move with meaning.",
    "A true fighter is never angry.",
    "Mastery is in the mundane. Discipline is the path.",
    "The best fight is the one you walk away from having never thrown a punch."
]
weapon_quotes = [
    "Precision beats power. Timing beats speed.",
    "Strike with purpose, move with meaning.",
...
if category == "Weapon":
    ...
    quote = random.choice(martial_quotes)
elif category == "Grappling":
    quote = random.choice(grappling_quotes)
elif category == "Unarmed":
    quote = random.choice(unarmed_quotes)
...
if st.button("Roll Weapon Move"): result = df[df["Category"] == "Weapon"].sample(1).iloc[0] 
display_move(result, "Weapon Move") st.session_state['last_moves'] = pd.DataFrame([result])

if st.button("Roll Grappling Move"): result = df[df["Category"] == "Grappling"].sample(1).iloc[0] 
display_move(result, "Grappling Move") st.session_state['last_moves'] = pd.DataFrame([result])

if st.button("Roll One Unarmed Move"): result = df[df["Category"] == "Unarmed"].sample(1).iloc[0] 
display_move(result, "Unarmed Move") st.session_state['last_moves'] = pd.DataFrame([result])

if st.button("Roll Two Unarmed Moves"): results = df[df["Category"] == "Unarmed"].sample(2) st.session_state['last_moves'] = results.reset_index(drop=True) 
for i, (_, result) in enumerate(results.iterrows(), 1): display_move(result, f"Unarmed Move {i}")

st.info("Click any button to generate a martial arts move!")

Download full dataset

st.download_button( label="Download Full Move List as CSV", data=df.to_csv(index=False).encode('utf-8'), file_name='moves.csv', mime='text/csv' )

Download last rolled moves

if st.session_state['last_moves'].shape[0] > 0: st.download_button( label="Download Last Rolled Moves", data=st.session_state['last_moves'].to_csv(index=False).encode('utf-8'), file_name='last_moves.csv', mime='text/csv' )
