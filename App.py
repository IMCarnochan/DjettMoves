...
def display_move(move, title): st.subheader(title) st.write(f"Martial Art: {move['Martial Art']}") st.write(f"Move: {move['Official Move Name']}") st.write(f"Striking Limb: {move['Striking Limb']}") st.write(f"Speed: {move['Speed (1–10)']}") st.write(f"Power: {move['Power (1–10)']}") st.write(f"Element: {move['Element']}") st.markdown(f"Formal Move Name: {move['Formal Move Name']}")

if st.button("Roll Weapon Move"): move = df[df["Category"] == "Weapon"].sample(1).iloc[0] display_move(move, "Weapon Move")

if st.button("Roll Grappling Move"): move = df[df["Category"] == "Grappling"].sample(1).iloc[0] display_move(move, "Grappling Move")

if st.button("Roll One Unarmed Move"): move = df[df["Category"] == "Unarmed"].sample(1).iloc[0] display_move(move, "Unarmed Move")

if st.button("Roll Two Unarmed Moves"): moves = df[df["Category"] == "Unarmed"].sample(2) for i, (_, move) in enumerate(moves.iterrows(), 1): display_move(move, f"Unarmed Move {i}")
